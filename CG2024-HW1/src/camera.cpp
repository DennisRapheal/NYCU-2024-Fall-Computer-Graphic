...
void Camera::updateViewMatrix() {
  ...
  // 1. Rotate original_front and original_up using this->rotation.
  front = normalize(rotation * original_front);
  up = normalize(rotation * original_up);

  // Step 2: calculate right vector by cross product.
  right = glm::normalize(glm::cross(front, up));
  up = glm::normalize(glm::cross(right, front));

  // Step 3: Calculate view matrix with position.
